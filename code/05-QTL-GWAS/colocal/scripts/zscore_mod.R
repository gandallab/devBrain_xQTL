...
eqtl_in_gwas <- eqtl %>% filter(V1 %in% gwas$V1)
gwas_in_eqtl <- gwas %>% filter(V1 %in% eqtl$V1)
...
# > is.unsorted(bim$V4)
# [1] FALSE
data_bim <- fread("/u/project/gandalm/cindywen/isoform_twas/genotype/all_data/isec_R2_greater_than_3/ancestry/filtered.hg19.sorted.removeGeneOutlier.bim", data.table = F)
# > dim(bim)
# [1] 480110      6
# > dim(data_bim)
# [1] 8420206       6
...
write.table(gwas_in_eqtl, paste0(
    ...
    args$level, "_ieqtl/",
    args$trait, "/locus_", args$locus, "/", 
    args$module_gene, "_gwas_zscore.txt"
),
col.names = F, row.names = F, quote = F, sep = "\t"
)
...
write.table(as.data.frame(gwas_in_eqtl$V1), paste0(
    ...
    args$level, "_ieqtl/",
    args$trait, "/locus_", args$locus, "/", 
    args$module_gene, "_gwas_snplist.txt"
),
col.names = F, row.names = F, quote = F, sep = "\t"
)
