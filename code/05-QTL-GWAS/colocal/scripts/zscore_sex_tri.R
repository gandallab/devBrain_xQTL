...
p <- add_argument(p, "--level", help = "eqtl, sqtl")
...
eqtl <- fread(paste0(
    ...
    args$group, "_", args$level, "/",
    args$trait, "/locus_", args$locus, "/", args$gene, "_all_pairs.txt"
),
data.table = F
)
...
# > is.unsorted(bim$V4)
# [1] FALSE
if (args$group %in% c("male", "female")) {
    # ALL multi-ethnic genotype    
    data_bim <- fread(
...
} else if (args$group %in% c("tri1", "tri2")) {
   # EUR genotype    
    data_bim <- fread(
    "/u/project/gandalm/cindywen/isoform_twas/genotype/all_data/isec_R2_greater_than_3/ancestry/eur/filtered.hg19.sorted.removeGeneOutlier.bim",
    data.table = F
)
}
...
write.table(eqtl_in_gwas, paste0(
    "/u/project/gandalm/cindywen/isoform_twas/colocal/results_",
    args$group, "_", args$level, "/",
    args$trait, "/locus_", args$locus, "/", args$gene, "_qtl_zscore.txt"
),
