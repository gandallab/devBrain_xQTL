...
p <- add_argument(p, "--level", help = "")


args <- parse_args(p)

# 1. read input files
eqtl <- fread(paste0("/u/project/gandalm/cindywen/isoform_twas/colocal/results_", args$level, "/", args$trait, "/locus_", args$locus, "/", args$gene, "_all_pairs.txt"), data.table = F)
...
write.table(eqtl_in_gwas, paste0(
    "/u/project/gandalm/cindywen/isoform_twas/colocal/results_", args$level, "/",
