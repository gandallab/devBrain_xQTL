...
list=./cindywen/isoform_twas/FINAL/libd.phase2hippo.fastq.txt
INLINE=`head -n ${SGE_TASK_ID} ${list} | tail -n 1`
IFS=$'\t' params=($INLINE)
SUBJECT_ID=${params[1]}             #BrXXXX
FASTQ_PREFIX=${params[0]}           #RXXXX
...
touch ${CONCAT_PATH}/${SUBJECT_ID}.hippo.done
