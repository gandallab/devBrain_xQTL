# Refer to eQTL Catalogue susie-workflow
# Made a few minor changes. Also note the original script takes in association file of all feature-topSNP from permutations; here only the significant features are input
...
option_list <- list(
  ...
  optparse::make_option(c("--sample_meta"), type="character", default=NULL,
                        help="Sample metadata file path of samples used in expression-matrix. Tab separated", metavar = "type"),
  optparse::make_option(c("--expression_matrix"), type="character", default=NULL,
                        help="Expression matrix file path with gene phenotype-id in rownames and sample-id in columnnames", metavar = "type"),
  # optparse::make_option(c("--phenotype_list"), type="character", default=NULL,
  #                       help="Path to the phenotype list file.", metavar = "type"),
  optparse::make_option(c("--sig_pheno"), type="character", default=NULL,
                        help="Path to the perm significant output.", metavar = "type"),
...
storage.mode(covariates_matrix) <- "numeric"
...
#   message("Number of phenotypes included for analysis: ", nrow(phenotype_list))
# }
phenotype_table = read.table(opt$sig_pheno, check.names = F, header = T, stringsAsFactors = F)
phenotype_table = phenotype_table %>% dplyr::mutate(group_id = pid)
phenotype_list = dplyr::semi_join(phenotype_meta, phenotype_table, by = "group_id")
message("Number of phenotypes included for analysis: ", nrow(phenotype_list))
