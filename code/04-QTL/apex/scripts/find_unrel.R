...
argp <- add_argument(argp, "--kinship_file", help="KING ibdseg matrix RData")
argp <- add_argument(argp, "--divergence_file", help="KING robust output")
argp <- add_argument(argp, "--kinship_threshold", help="")
argp <- add_argument(argp, "--divergence_threshold", help="")
argp <- add_argument(argp, "--out_rel_file", help="Related out")
argp <- add_argument(argp, "--out_unrel_file", help="Unrelated out")
...
# Any pair of individuals with a pairwise kinship greater than kin.thresh will be declared 'related'. Any pair of individuals with a pairwise divergence measure less than div.thresh will be declared ancestrally 'divergent'.
kin_thresh <- as.numeric(argv$kinship_threshold)
div_thresh <- as.numeric(argv$divergence_threshold)
...
# Note: we don't have population as study; the algorithm will find an optimal partition
study.unrel <- NULL
...
rels <- part$rels
unrels <- part$unrels
save(rels, file=argv$out_rel_file)
save(unrels, file=argv$out_unrel_file)
