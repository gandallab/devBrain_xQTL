...
# keep both gene_id and gene_name
# one gene_name can have multiple gene_id
# in such cases, gene_id1 gene_id2 gene_name gene_name in output df
map_clusters_to_genes <- function(intron_meta, exons_table) {
  gene_df <- foreach (chr = sort(unique(intron_meta$chr)), .combine = rbind) %dopar% {
    ...
    all_matches <- rbind(three_prime_matches, five_prime_matches)[ , c("clu", "gene_id", "gene_name")]
  ...
  clu_df <- gene_df %>% group_by(clu) %>% summarize(genes = paste(gene_id, collapse = ","), names = paste(gene_name, collapse = ","))
