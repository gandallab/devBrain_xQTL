...
# CW edit:
# Generate all_exons file with gene_id for GTEx way of annotating intron cluster and mapping with grouping
# Original leafcutter annotation codes are fine for leafviz way of annotating, but do not have gene_id in all_exons, which can be problematic for GTEx pipeline, as there are duplicated gene_names with different gene_id
...
if (keys %hOut){
   ...
   print ALLEXONS "chr\tstart\tend\tstrand\tgene_id\tgene_name\n";
   ...
   foreach my $sTranscriptID (keys %hOut){
      ...
      foreach my $rExon (@aE){
         ...
         print ALLEXONS join ("\t", $hT{'chr'}, $nStart, $nEnd, $hT{'strand'}, $hT{'gene_id'}, $hT{'gene_name'}), "\n";
      }
